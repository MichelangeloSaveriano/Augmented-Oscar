...
def make_data_loader(args, yaml_file, tokenizer, is_distributed=True, 
        is_train=True, indexes=None):
    if indexes is not None:
      dataset = torch.utils.data.Subset(
        build_dataset(yaml_file, tokenizer, args,is_train=(is_train and not args.scst)), indexes) 
    else:
...
def save_checkpoint(model, tokenizer, args, epoch, iteration, num_trial=10):
    ...
    for i in range(num_trial):
        try:
            ...
            torch.save(model.gcn.state_dict(), checkpoint_dir + '/gcn.ckpt')
...
def evaluate(args, val_dataloader, model, tokenizer, output_dir):
    ...
    if isinstance(val_dataloader.dataset, torch.utils.data.Subset):      
      predict_file = get_predict_file(output_dir,
            val_dataloader.dataset.dataset.yaml_file, args)
    else:      
      predict_file = get_predict_file(output_dir,
    ...
    if isinstance(val_dataloader.dataset, torch.utils.data.Subset):
      caption_file = val_dataloader.dataset.dataset.get_caption_file_in_coco_format()
      data = val_dataloader.dataset.dataset.yaml_file.split('/')[-2]
    else:
...
def box_distance_adjacency(bbox_features, tau=0.05):
  bbox_features = bbox_features[:, :-2] 
  distance_matrix = torch.zeros((bbox_features.shape[0], bbox_features.shape[0]))
  for i,( x1_left, y1_bottom, x1_right, y1_top) in enumerate(bbox_features):
    temp_list = []
    for j, (x2_left, y2_bottom, x2_right, y2_top) in enumerate(bbox_features):
      y_top    = torch.tensor([y1_top,    y2_top   ])
      y_bottom = torch.tensor([y1_bottom, y2_bottom])
      x_right  = torch.tensor([x1_right,  x2_right ])
      x_left   = torch.tensor([x1_left,   x2_left  ])
      y_dist = F.relu(y_bottom[torch.argmax(y_top)] - torch.min(y_top) + tau)
      x_dist = F.relu(x_left[torch.argmax(x_right)] - torch.min(x_right) + tau)

      dist = torch.sqrt(x_dist**2 + y_dist**2)
      if torch.any(torch.tensor([x2_left, y2_bottom, x2_right, y2_top]) != 0) and torch.any(torch.tensor([x1_left, y1_bottom, x1_right, y1_top]) != 0) :
            distance_matrix[i,j] = 1 - dist
  return distance_matrix


def absolute_cosine_distance_matrix(embeddings, tau=0.5):
  embeddings = F.normalize(embeddings, dim=-1)
  abs_cos_dist = torch.abs(torch.mm(embeddings, embeddings.t()))
  return abs_cos_dist * (abs_cos_dist > tau)


def create_single_graph(input_ids, img_feats):
  box_adjacency = box_distance_adjacency(img_feats[:,-6:])
  image_adjacency = absolute_cosine_distance_matrix(img_feats[:, :-6])
  rand_adjacency = torch.multinomial(torch.tensor([0.8, 0.2]),
                                     torch.prod(torch.tensor(box_adjacency.shape)), 
                                     replacement=True).reshape(box_adjacency.shape)
  final_adjacency = (box_adjacency.to('cuda') + 
                     image_adjacency.to('cuda') + 
                     rand_adjacency.to('cuda'))/3
  return dense_to_sparse(final_adjacency)[0]


class GCN(torch.nn.Module):
    '''
    Implements a simple GCN with dropout and residual connections  
    '''
    def __init__(self, in_features, hidden_features, output_features,
                 num_blocks, p_dropout = 0.6):
        super().__init__()
        
        layer_list = [gnn.GCNConv(in_features, hidden_features)]
        for i in range(num_blocks):
          layer_list.append(gnn.GCNConv(hidden_features, hidden_features))
        layer_list.append(gnn.GCNConv(hidden_features, output_features))

        self.gcn = nn.Sequential(*layer_list)
    ...
    def forward(self, data):
        ...
        for l in self.gcn:
          x_residual = x
          x = l(x, edge_index = edge)
          x = F.relu(x)
          x = x_residual + F.dropout(x, training = self.training, p=self.p_dropout)
        return x


class AugOscar(BertForImageCaptioning):
  '''
  Implements the Augmented Oscar model by combining 
  the original Oscar model with a Graph Convolutional Network
  '''
  def __init__(self, config):
    super(AugOscar, self).__init__(config)
    self.config = config
    if not config.from_oscar:
      self.oscar = BertForImageCaptioning(config)
    else:
      self.oscar = BertForImageCaptioning.from_pretrained(config.model_name_or_path)
    
    self.gcn = GCN(in_features=2054, hidden_features=2054, 
                  output_features=2054, num_blocks=1, p_dropout=0.6)
    # if config.gcn_path is not None:
    #   self.gcn.load_state_dict(torch.load(config.gcn_path))

  def forward(self, *args, **kwargs):
    if not self.config.skip_gcn:
      input_ids = kwargs.get('input_ids')
      img_feats = kwargs.get('img_feats')
      new_img_feats = torch.stack([self.gcn(Data(edge_index=create_single_graph(ids, img), x=img).to('cuda')) for ids, img in 
                                      zip(input_ids, img_feats)])
      kwargs['img_feats'] = new_img_feats
    return self.oscar(*args, **kwargs)    

  def load_gcn_parameters(self, gcn_path):
    self.gcn.load_state_dict(torch.load(gcn_path))
...
def main():
    ...
    parser.add_argument("--train_size", default=10000, type=int, 
                        help="Size of the subset used for training")
    parser.add_argument("--test_size", default=1000, type=int, 
                        help="Size of the subset used for testing")
    parser.add_argument("--val_size", default=1000, type=int, 
                        help="Size of the subset used for validation")
    parser.add_argument("--skip_gcn", action='store_true', help="Whether to skip the GCN.")
    parser.add_argument("--from_oscar", action='store_true', help="Whether to skip the GCN.")
    ...
    # Custom args
    gcn_path = args.eval_model_dir + '/gcn.ckpt'
    train_rows = list(range(args.train_size))
    test_rows = list(range(args.test_size))
    val_rows = list(range(args.val_size))
    ...
    config_class, model_class, tokenizer_class = BertConfig, AugOscar, BertTokenizer
    if args.do_train:
        ...
        config.skip_gcn = args.skip_gcn
        config.from_oscar = args.from_oscar
        ...
        if args.model_name_or_path:
          model.load_gcn_parameters(args.model_name_or_path+ 'gcn.ckpt')
        else:
          model.load_gcn_parameters(gcn_path)
    else:
        checkpoint = args.eval_model_dir
    ...
    if args.do_train:
        train_dataloader = make_data_loader(args, args.train_yaml, tokenizer,
            args.distributed, is_train=True, indexes = train_rows)
          
        print('Length of train set:' ,len(train_dataloader.dataset))

        val_dataloader = None
        if args.evaluate_during_training:
            val_dataloader = make_data_loader(args, args.val_yaml, tokenizer,
                args.distributed, is_train=False, indexes=val_rows)

            print('Length of val set:' ,len(val_dataloader.dataset))
        ...
        if args.do_test:
            ...
            test_dataloader = make_data_loader(args, args.test_yaml, 
                tokenizer, args.distributed, is_train=False, indexes = test_rows)
            print('Length of test set:' ,len(test_dataloader.dataset))
